...
<div class="flex-1 px-4 md:px-8 py-7"
     x-data="{
        ...
        mode: 'tambah',
        form: { id: null, kode: '', nama: '', keterangan: '', urutan: '', is_aktif: true },
        buka(mode, data = null) {
            this.mode = mode;
            this.form = data
                ? { ...data }
                : { id: null, kode: '', nama: '', keterangan: '', urutan: '', is_aktif: true };
            this.showForm = true;
        }
     }">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-800">Akun Pembiayaan</h1>
            <p class="text-xs text-slate-400 mt-0.5">Mata anggaran yang membebani perjalanan dinas pada daftar nominatif</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold text-slate-800">{{ $totalAkun }}</p>
                <p class="text-xs text-slate-400">Total Akun</p>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400">Ditawarkan pada Nominatif</p>
            </div>
        </div>
    </div>
    ...
    <div x-show="showForm" x-transition x-cloak class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-5">
        <h3 class="font-bold text-slate-800 text-sm mb-4"
            x-text="mode === 'tambah' ? 'Tambah Akun Pembiayaan' : 'Ubah Akun Pembiayaan'"></h3>

        <form method="POST"
              :action="mode === 'tambah'
                        ? '{{ route('master.akun-pembiayaan.store') }}'
                        : '{{ url('master/akun-pembiayaan') }}/' + form.id">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Kode Akun <span class="text-red-500">*</span></label>
                    <input type="text" name="kode" x-model="form.kode" required placeholder="524111"
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nama Akun <span class="text-red-500">*</span></label>
                    <input type="text" name="nama" x-model="form.nama" required placeholder="Belanja Perjalanan Dinas Biasa"
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-400 focus:border-transparent transition">
                    @error('nama') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Keterangan</label>
                    <input type="text" name="keterangan" x-model="form.keterangan" placeholder="Boleh dikosongkan"
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-400 focus:border-transparent transition">
            ...
            <div class="flex items-center justify-between gap-3 mt-4">
                <label class="flex items-center gap-2 text-sm text-slate-600 select-none cursor-pointer">
                    ...
                    Tawarkan pada daftar nominatif
                </label>
    ...
    <form method="GET" action="{{ route('master.akun-pembiayaan') }}">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-5 flex flex-col sm:flex-row gap-3">
            ...
            @if ($search)
                <a href="{{ route('master.akun-pembiayaan') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold rounded-xl transition text-center">Reset</a>
            @endif
        </div>
    </form>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100">
            <p class="text-sm font-bold text-slate-700">
                Daftar Akun
                <span class="ml-2 text-xs font-semibold text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">{{ $akun->total() }} data</span>
            </p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-left text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5">Nama Akun</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse ($akun as $item)
                        <tr class="hover:bg-slate-50/60 transition">
                            ...
                            <td class="px-4 py-4">
                                <span class="inline-block text-xs font-bold px-2.5 py-1 rounded-full bg-blue-100 text-blue-700">{{ $item->kode }}</span>
                            </td>
                            <td class="px-4 py-4 font-semibold text-slate-700">{{ $item->nama }}</td>
                            <td class="px-4 py-4 text-slate-500 text-xs">{{ $item->keterangan ?? '—' }}</td>
                            <td class="px-4 py-4 text-center">
                                <span class="inline-block text-xs font-bold px-2 py-1 rounded-full bg-slate-100 text-slate-600">{{ $item->nominatif_count }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    <button @click="buka('ubah', {{ Js::from($item->only(['id', 'kode', 'nama', 'keterangan', 'urutan', 'is_aktif'])) }})"
                                    ...
                                    <form method="POST" action="{{ route('master.akun-pembiayaan.destroy', $item) }}"
                                          x-data
                                          @submit.prevent="if (confirm('Hapus akun {{ addslashes($item->kode) }}?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <p class="text-sm text-slate-400">Belum ada akun pembiayaan</p>
                                <button @click="buka('tambah')" class="text-sm text-teal-600 font-semibold hover:underline mt-1">+ Tambah Akun Pertama</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
