...
<div class="flex-1 px-4 md:px-8 py-7"
     x-data="{
        ...
        mode: 'tambah',
        form: { id: null, kode: '', nama: '', keterangan: '', is_aktif: true },
        buka(mode, data = null) {
            this.mode = mode;
            this.form = data
                ? { ...data }
                : { id: null, kode: '', nama: '', keterangan: '', is_aktif: true };
            this.showForm = true;
        }
     }">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-800">Unit Kerja</h1>
            <p class="text-xs text-slate-400 mt-0.5">Referensi unit, jurusan, dan bagian di lingkungan Poltekkes</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold text-slate-800">{{ $totalUnit }}</p>
                <p class="text-xs text-slate-400">Total Unit Kerja</p>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400">Unit Aktif</p>
            </div>
        </div>
    </div>
    ...
    <div x-show="showForm" x-transition x-cloak class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-5">
        <h3 class="font-bold text-slate-800 text-sm mb-4"
            x-text="mode === 'tambah' ? 'Tambah Unit Kerja Baru' : 'Ubah Unit Kerja'"></h3>

        <form method="POST"
              :action="mode === 'tambah'
                        ? '{{ route('master.unit-kerja.store') }}'
                        : '{{ url('master/unit-kerja') }}/' + form.id">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div>
                    ...
                    <input type="text" name="kode" x-model="form.kode" required maxlength="50" placeholder="ADUM"
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm uppercase focus:ring-2 focus:ring-teal-400 focus:border-transparent transition">
                    @error('kode') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="md:col-span-2">
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nama Unit <span class="text-red-500">*</span></label>
                    <input type="text" name="nama" x-model="form.nama" required placeholder="Bagian Administrasi Umum"
                ...
                <div>
                    ...
                    <input type="text" name="keterangan" x-model="form.keterangan" placeholder="Opsional"
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-400 focus:border-transparent transition">
    ...
    {{-- Search --}}
    <form method="GET" action="{{ route('master.unit-kerja') }}">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-5">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari kode atau nama unit..."
                ...
                @if ($search)
                    <a href="{{ route('master.unit-kerja') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold rounded-xl transition">Reset</a>
                @endif
            </div>
        </div>
    </form>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100">
            <p class="text-sm font-bold text-slate-700">
                Daftar Unit Kerja
                <span class="ml-2 text-xs font-semibold text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">{{ $unitKerja->total() }} data</span>
            </p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-left text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5 w-32">Kode</th>
                        <th class="text-left text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5">Nama Unit</th>
                        <th class="text-center text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5 w-28">Pegawai</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse ($unitKerja as $item)
                        <tr class="hover:bg-slate-50/60 transition">
                            ...
                            <td class="px-4 py-4">
                                <span class="inline-block text-xs font-bold px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700">{{ $item->kode }}</span>
                            </td>
                            <td class="px-4 py-4">
                                <p class="font-semibold text-slate-800">{{ $item->nama }}</p>
                                @if ($item->keterangan)
                                    <p class="text-xs text-slate-400 mt-0.5">{{ $item->keterangan }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span class="inline-block text-xs font-bold px-2 py-1 rounded-full bg-slate-100 text-slate-600">{{ $item->pegawai_count }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    <button @click="buka('ubah', {{ Js::from($item->only(['id', 'kode', 'nama', 'keterangan', 'is_aktif'])) }})"
                                    ...
                                    <form method="POST" action="{{ route('master.unit-kerja.destroy', $item) }}"
                                          x-data
                                          @submit.prevent="if (confirm('Hapus unit kerja {{ addslashes($item->nama) }}?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm text-slate-400">Belum ada unit kerja</p>
                                    <button @click="buka('tambah')" class="text-sm text-teal-600 font-semibold hover:underline">+ Tambah Unit Pertama</button>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <x-pagination :paginator="$unitKerja" />
    </div>
