...
<div class="flex-1 px-4 md:px-8 py-7"
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-800">Lokasi Tujuan</h1>
            <p class="text-xs text-slate-400 mt-0.5">Referensi kota tujuan perjalanan dinas beserta klasifikasinya</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold text-slate-800">{{ $totalLokasi }}</p>
                <p class="text-xs text-slate-400">Total Lokasi Tujuan</p>
            </div>
        </div>
    </div>
    ...
    <div x-show="showForm" x-transition x-cloak class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-5">
        <h3 class="font-bold text-slate-800 text-sm mb-4"
            x-text="mode === 'tambah' ? 'Tambah Lokasi Tujuan' : 'Ubah Lokasi Tujuan'"></h3>

        <form method="POST"
              :action="mode === 'tambah'
                        ? '{{ route('master.lokasi.store') }}'
                        : '{{ url('master/lokasi') }}/' + form.id">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nama Kota <span class="text-red-500">*</span></label>
                    <input type="text" name="nama" x-model="form.nama" required placeholder="Manado"
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Provinsi</label>
                    <input type="text" name="provinsi" x-model="form.provinsi" placeholder="Sulawesi Utara"
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-400 focus:border-transparent transition">
                    @error('provinsi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Klasifikasi <span class="text-red-500">*</span></label>
    ...
    {{-- Search & Filter --}}
    <form method="GET" action="{{ route('master.lokasi') }}">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-5">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari kota atau provinsi..."
                ...
                @if ($search || $jenis)
                    <a href="{{ route('master.lokasi') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold rounded-xl transition">Reset</a>
                @endif
            </div>
        </div>
    </form>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100">
            <p class="text-sm font-bold text-slate-700">
                ...
                <span class="ml-2 text-xs font-semibold text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">{{ $lokasi->total() }} data</span>
            </p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-left text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5">Kota</th>
                        <th class="text-left text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5">Provinsi</th>
                        <th class="text-center text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5 w-36">Klasifikasi</th>
                        <th class="text-center text-xs font-bold text-slate-500 uppercase tracking-wide px-4 py-3.5 w-28">Dipakai</th>
                    ...
                    @forelse ($lokasi as $item)
                        <tr class="hover:bg-slate-50/60 transition">
                            <td class="px-4 py-4 text-center text-xs font-semibold text-slate-400">
                                {{ ($lokasi->currentPage() - 1) * $lokasi->perPage() + $loop->iteration }}
                            </td>
                            <td class="px-4 py-4 font-semibold text-slate-800">{{ $item->nama }}</td>
                            <td class="px-4 py-4 text-slate-500">{{ $item->provinsi ?? '—' }}</td>
                            <td class="px-4 py-4 text-center">
                                <span class="inline-block text-xs font-bold px-2.5 py-1 rounded-full bg-cyan-100 text-cyan-700">{{ $item->jenis_label }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <form method="POST" action="{{ route('master.lokasi.destroy', $item) }}"
                                          x-data
                                          @submit.prevent="if (confirm('Hapus lokasi {{ addslashes($item->nama) }}?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm text-slate-400">Belum ada lokasi tujuan</p>
                                    <button @click="buka('tambah')" class="text-sm text-teal-600 font-semibold hover:underline">+ Tambah Lokasi Pertama</button>
                                </div>
                            </td>
                        </tr>
                    @endforelse
