...
@section('title', 'Kategori Pembiayaan')
...
<div class="flex-1 px-4 md:px-8 py-7"
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-800">Kategori Pembiayaan</h1>
            <p class="text-xs text-slate-400 mt-0.5">Sumber dana yang membiayai perjalanan dinas — RM, BLU, LN, dan seterusnya</p>
        </div>
    ...
    <div x-show="showForm" x-transition x-cloak class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-5">
        <h3 class="font-bold text-slate-800 text-sm mb-4"
            x-text="mode === 'tambah' ? 'Tambah Kategori Pembiayaan' : 'Ubah Kategori Pembiayaan'"></h3>

        <form method="POST"
              :action="mode === 'tambah'
                        ? '{{ route('master.kategori-pembiayaan.store') }}'
                        : '{{ url('master/kategori-pembiayaan') }}/' + form.id">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div>
                    ...
                    <input type="text" name="kode" x-model="form.kode" required placeholder="RM"
                ...
                <div>
                    ...
                    <input type="text" name="nama" x-model="form.nama" required placeholder="Rupiah Murni"
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-400 focus:border-transparent transition">
    ...
    <form method="GET" action="{{ route('master.kategori-pembiayaan') }}">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-5 flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari kode atau nama kategori..."
            ...
            @if ($search)
                <a href="{{ route('master.kategori-pembiayaan') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold rounded-xl transition text-center">Reset</a>
            @endif
        </div>
    </form>
                        ...
                        <tr class="hover:bg-slate-50/60 transition">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <form method="POST" action="{{ route('master.kategori-pembiayaan.destroy', $item) }}"
                                          x-data
                                          @submit.prevent="if (confirm('Hapus kategori {{ addslashes($item->kode) }}?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <p class="text-sm text-slate-400">Belum ada kategori pembiayaan</p>
                                <button @click="buka('tambah')" class="text-sm text-teal-600 font-semibold hover:underline mt-1">+ Tambah Kategori Pertama</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
