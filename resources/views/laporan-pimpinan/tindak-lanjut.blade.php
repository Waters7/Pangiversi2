...
@section('title', 'Tindak Lanjut Laporan Perjadin')
...
<div class="flex-1 px-4 md:px-8 py-7">
    ...
    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-800">Tindak Lanjut</h1>
        <p class="text-xs text-slate-400 mt-0.5">
            Rencana tindak lanjut dari seluruh laporan perjalanan dinas yang sudah dikirim pelaksana
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-red-100 text-red-700 flex items-center justify-center shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 9v4m0 4h.01M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/></svg>
            </div>
            <div>
                <p class="text-2xl font-bold text-slate-800">{{ $jumlahTerlambat }}</p>
                <p class="text-xs text-slate-400">Lewat Target</p>
            </div>
        </div>
    </div>

    <form method="GET" action="{{ route('laporan-perjadin.tindak-lanjut') }}" class="mb-5">
                    ...
                    <div class="flex flex-wrap items-start gap-x-4 gap-y-2">
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-xs text-slate-400">
                                @if ($usulan)
                                    <span class="font-semibold text-slate-600">{{ $usulan->user?->nama }}</span>
                                    <a href="{{ route('laporan-perjadin.show', $usulan->no_usulan) }}"
                        ...
                        {{-- Pimpinan memantau, pelaksana yang memperbarui: status di
                             sini hanya dibaca, supaya kemajuannya tetap laporan pelaksana. --}}
                        <span class="inline-block text-xs font-bold px-2.5 py-1 rounded-full shrink-0 {{ $item->status->badge() }}">
                            {{ $item->status->label() }}
                        </span>
                    </div>
                </div>
            @empty
                <div class="px-6 py-14 text-center">
                    ...
                    <p class="text-xs text-slate-400 mt-1">
                        Tindak lanjut muncul di sini setelah pelaksana mengirim laporannya.
                    </p>
                </div>
            @endforelse
        </div>
