...
<div class="flex-1 px-4 md:px-8 py-7" x-data="administrasi()">
    ...
    @php
        ...
        $subjudulHalaman = 'Akun, peran, penempatan, dan siapa yang sedang masuk';
    @endphp
    @include('administrasi.partials.kepala', ['aksiKepala' => new \Illuminate\Support\HtmlString(
        '<button @click="showAddModal = true" type="button" class="flex items-center gap-2 px-4 py-2.5 bg-teal-500 hover:bg-teal-600 text-white text-sm font-semibold rounded-xl transition shadow-sm self-start">'
        .'<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Tambah Pengguna</button>'
    )])
