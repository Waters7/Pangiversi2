...
<head>
    ...
    <style>
        * { font-family: DejaVu Sans, sans-serif; }
        ...
        table.ttd td { width: 50%; vertical-align: top; font-size: 11px; }
        .ruang { height: 55px; }
...
<body>
    ...
    <table class="biaya">
        ...
        <tbody>
            ...
            {{-- Tiap komponen tampil sebagai barisnya sendiri, bernomor urut.
                 Pengelompokan per kategori dilepas karena menyembunyikan
                 komponen tunggal — sedangkan dokumen ini justru harus
                 memperlihatkan semuanya, lengkap dengan kode bookingnya. --}}
            @forelse ($rincianPerKategori as $kategori => $baris)
        ...
        <tr>
            <td>
                ...
                @if ($qrBendahara)
                    ...
                    NIP : {{ $bendahara?->nip ?? '…………………………' }}<br>
                    <span class="kode-qr">{{ $keuangan->kode_konfirmasi_bayar }}</span>
                    <p class="catatan-qr">
            ...
            <td>
                ...
                @if ($qrPelaksana)
                    ...
                    NIP : {{ $peserta?->nip ?? $usulan->user?->nip ?? '-' }}<br>
                    <span class="kode-qr">{{ $daftarRiil->kode_konfirmasi }}</span>
                    <p class="catatan-qr">
            ...
            <tr>
                ...
                <td>
                    ...
                    @if ($qrPpk)
                        ...
                        NIP : {{ $daftarRiil->ppk?->nip ?? $ppk?->nip ?? '…………………………' }}<br>
                        <span class="kode-qr">{{ $daftarRiil->kode_verifikasi }}</span>
                        <p class="catatan-qr">
                    ...
                    @else
                        <div class="ruang"></div>
                        <span class="nama">{{ $ppk?->nama ?? '……………………………' }}</span><br>
                        NIP : {{ $ppk?->nip ?? '…………………………' }}
                    @endif
                </td>
            </tr>
